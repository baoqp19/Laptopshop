...
import vn.laptopshop.laptopshop.domain.dto.ProductCriteriaDTO;
...
public class ProductService {
    ...
    public Page<Product> fetchProductsWithSpec(Pageable page, ProductCriteriaDTO productCriteriaDTO) {
        ...
        if (productCriteriaDTO.getTarget() != null && productCriteriaDTO.getTarget().isPresent()) {
            Specification<Product> currentSpecs = ProductSpect.matchListTarget(productCriteriaDTO.getTarget().get());
            combinedSpec = combinedSpec.and(currentSpecs);
        }
        if (productCriteriaDTO.getFactory() != null && productCriteriaDTO.getFactory().isPresent()) {
            Specification<Product> currentSpecs = ProductSpect.matchListFactory(productCriteriaDTO.getFactory().get());
            combinedSpec = combinedSpec.and(currentSpecs);
        }
    ...
    public Specification<Product> buildPriceSpecification(List<String> price) {
        ...
        for (String p : price) {
            ...
            if (min != 0 && max != 0) {
                Specification<Product> rangeSpec = ProductSpect.matchMultiplePrice(min, max);
